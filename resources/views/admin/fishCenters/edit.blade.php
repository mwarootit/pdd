        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.fishCenter.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.fish-centers.update", [$fishCenter->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('island') ? 'has-error' : '' }}">
                            <label for="island">{{ trans('cruds.fishCenter.fields.island') }}</label>
                            <input class="form-control" type="text" name="island" id="island" value="{{ old('island', $fishCenter->island) }}">
                        ...
                        <div class="form-group {{ $errors->has('operated_by') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="operated_by" id="operated_by">
                                ...
                                @foreach(App\Models\FishCenter::OPERATED_BY_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('operated_by', $fishCenter->operated_by) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('date_of_recieval') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="date_of_recieval" id="date_of_recieval" value="{{ old('date_of_recieval', $fishCenter->date_of_recieval) }}">
                        ...
                        <div class="form-group {{ $errors->has('item') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="item" id="item" value="{{ old('item', $fishCenter->item) }}">
                            @if($errors->has('item'))
                        ...
                        <div class="form-group {{ $errors->has('quantity') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="quantity" id="quantity" value="{{ old('quantity', $fishCenter->quantity) }}">
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="status" id="status">
                                ...
                                @foreach(App\Models\FishCenter::STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $fishCenter->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('date_of_monitoring') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="date_of_monitoring" id="date_of_monitoring" value="{{ old('date_of_monitoring', $fishCenter->date_of_monitoring) }}">
                        ...
                        <div class="form-group {{ $errors->has('remarks') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="remarks" id="remarks" value="{{ old('remarks', $fishCenter->remarks) }}">
